//
// "$Id: fl_read_image.cpp 6065 2008-03-09 17:58:10Z matt $"
//
...
/* #include "flstring.h" don't know where this is, may have to create it */
#include "fixes.h"

#ifdef debugging
#define DEBUG "hell, yes!"
#endif
...
static inline int
flocal_subimage_offsets(int a, int aw, int b, int bw, int &obw)
{
  int off;
  int ob;
...
static int localxgetimageerrhandler(Display *display, XErrorEvent *error)
{
...
flocal_read_image(uchar *p,		// I - Pixel buffer or NULL to allocate
...
#  ifdef __sgi
  printf("Using sgi XReadDisplayQueryExtension\n");
...
#  endif // __sgi
  ...
  if (!image) {
    ...
    if (win) {
      ...
      printf("Screen dimensions %d, %d, %d, %d\n",sx, sy, sw, sh);
      printf("ww %d, wh %d\n",ww,wh);
      if (ww < sw) sw = --ww;
      if (wh < sh) sh = --wh;
      printf("sw %d, sh %d\n",sw,sh);
    ...
    if (!win || (dx >= sx && dy >= sy && dx + w <= sw && dy + h <= sh)) {
      ...
      XErrorHandler old_handler = XSetErrorHandler(localxgetimageerrhandler);
      printf("Doing XGetImage on %ld - %d, %d, %d, %d\n", (long)fl_window, X, Y, w, h);
	...
	if (!image){
      ...
      noffx = flocal_subimage_offsets(sx, sw, dx, w, nw);
      noffy = flocal_subimage_offsets(sy, sh, dy, h, nh);
      printf("Thought image was crossing borders %d, %d, %d, %d\n",nw,nh,noffx,noffy);
      if (nw <= 0 || nh <= 0) return 0;

      // allocate the image
      int bpp = fl_visual->depth + ((fl_visual->depth / 8) % 2) * 8;
      char* buf = (char*)malloc(bpp / 8 * w * h);
      printf("flocal_read_image malloc %ld\n",(long)buf);
      image = XCreateImage(fl_display, fl_visual->visual,
	  fl_visual->depth, ZPixmap, 0, buf, w, h, bpp, 0);
      printf("Did XCreateImage %d, %d, returned %ld\n",w,h,(long)image);
      if (!image) {
	if (buf) free(buf);
	return 0;
  }
...
//
// End of "$Id: fl_read_image.cpp 6065 2008-03-09 17:58:10Z matt $".
//
