...
void indiUI::update() {
  ...
  char* gender;
  gender = strdup(""); // stops compiler whingeing about possibly uninitialised var
...
void statsUI::setvalues() {
  ...
  char buf[64];
#ifdef WIN32
  sprintf_s(buf,64,"%d",which->getindicount());
  indicount->value(buf);
  sprintf_s(buf,64,"%d",which->getfamcount());
  famcount->value(buf);
  sprintf_s(buf,64,"%d",which->getsourcount());
  srccount->value(buf);
  sprintf_s(buf,64,"%d",which->getrepocount());
  repocount->value(buf);
  sprintf_s(buf,64,"%d",which->getmaxindi());
  indimax->value(buf);
  sprintf_s(buf,64,"%d",which->getmaxfam());
  fammax->value(buf);
  sprintf_s(buf,64,"%d",which->getmaxsour());
  srcmax->value(buf);
  sprintf_s(buf,64,"%d",which->getmaxrepo());
  repomax->value(buf);
#else
...
// should not be in object.cpp. In fact, it probably belongs in gui.cpp
//
  ...
  famptr (found_fam)
{
  ...
  subitem = NULL;
  switch (*gender) {
  // we will crash and burn here if wife/husb don't have gender set
